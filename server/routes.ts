...
async function handleChatMessage(ws: WebSocket, data: any) {
  try {
    ...
    console.log(`WebSocket chat message received for session ${data.sessionId}`);
    
    // Save the user message to the database if persistence is enabled
    if (data.persist) {
      // First save user message
      const userMessage = await dbStorage.createChatMessage({
    ...
    let sessionMessages = [];
    if (data.sessionId && data.persist) {
      // Get previous messages if this is a persistent chat
      sessionMessages = await dbStorage.getChatMessages(data.sessionId);
    } else if (data.history && Array.isArray(data.history)) {
      // Use provided history for non-persistent chats
      sessionMessages = data.history;
    }
    
    // Send an immediate acknowledgment that we're processing
    ws.send(JSON.stringify({
      type: 'processing',
      messageId: data.messageId || Date.now()
    }));
    
    // Process with AI - stream tokens back via WebSocket
    try {
      // Process the chat message with AI
      const aiResponse = await generateChatResponse(data.content, sessionMessages);
      
      // Send the complete response first through WebSocket
      ws.send(JSON.stringify({
        type: 'chat-response',
        content: aiResponse,
        messageId: data.messageId || Date.now(),
      ...
      // If we should persist this chat, save the AI response
      if (data.persist && data.sessionId) {
        const savedMessage = await dbStorage.createChatMessage({
          sessionId: data.sessionId,
          role: 'assistant',
          content: aiResponse
        ...
        // Notify that the response was saved
        ws.send(JSON.stringify({
          type: 'chat-response-saved',
    ...
    ws.send(JSON.stringify({
      type: 'error',
      error: error instanceof Error ? error.message : 'Unknown error in chat processing'
    }));
  }
}

async function handleGrammarCheck(ws: WebSocket, data: any) {
  try {
    if (!data.text) {
      ws.send(JSON.stringify({
        type: 'error',
    ...
    console.log('WebSocket grammar check request');
    ...
    try {
      // First try with LanguageTool
      const language = data.language || 'en-US';
      const result = await checkGrammarWithLanguageTool(data.text, language);
      ...
      console.error('LanguageTool error, falling back to Perplexity:', languageToolError);
      
      // Fallback to Perplexity
      try {
        const result = await generateGrammarCheck(data.text);
        ws.send(JSON.stringify({
          type: 'grammar-result',
          result,
          messageId: data.messageId
    ...
    ws.send(JSON.stringify({
      type: 'error',
      error: error instanceof Error ? error.message : 'Unknown error in grammar check',
      messageId: data.messageId
    }));
  }
}

async function handleParaphrase(ws: WebSocket, data: any) {
  try {
    if (!data.text) {
      ws.send(JSON.stringify({
        type: 'error',
    ...
    console.log('WebSocket paraphrase request with style:', data.style);
    ...
    // Process paraphrase
    try {
      const result = await generateParaphrase(data.text, data.style || 'standard');
      
      // Send response
      ws.send(JSON.stringify({
        type: 'paraphrase-result',
        result: {
          paraphrasedText: result.paraphrased,
          metrics: result.metrics
        },
    ...
    ws.send(JSON.stringify({
      type: 'error',
      error: error instanceof Error ? error.message : 'Unknown error in paraphrase',
      messageId: data.messageId
    }));
  }
}

async function handleHumanize(ws: WebSocket, data: any) {
  try {
    if (!data.text) {
      ws.send(JSON.stringify({
        type: 'error',
    ...
    console.log('WebSocket humanize request with style:', data.style);
    ...
    // Process humanize
    try {
      const result = await generateHumanized(data.text, data.style || 'standard');
      
      // Send response
      ws.send(JSON.stringify({
        type: 'humanize-result',
        result: {
          humanizedText: result.humanized,
          metrics: result.metrics
        },
    ...
    ws.send(JSON.stringify({
      type: 'error',
      error: error instanceof Error ? error.message : 'Unknown error in humanize',
      messageId: data.messageId
    }));
  }
}

async function handleAiCheck(ws: WebSocket, data: any) {
  try {
    if (!data.text) {
      ws.send(JSON.stringify({
        type: 'error',
    ...
    console.log('WebSocket AI check request');
    ...
    // Process AI check
    try {
      const result = await checkAIContent(data.text);
    ...
    ws.send(JSON.stringify({
      type: 'error',
      error: error instanceof Error ? error.message : 'Unknown error in AI check',
      messageId: data.messageId
    }));
  }
}